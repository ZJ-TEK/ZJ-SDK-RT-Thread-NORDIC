...
static uint32_t bsp_qspi_flash_erase(nrf_qspi_erase_len_t length, uint32_t start_address)
{
    uint32_t err_code;
...
static uint32_t bsp_qspi_flash_write(void const * p_tx_buffer,
...
static uint32_t bsp_qspi_flash_read(void  * p_rx_buffer,
...
static void bsp_qspi_flash_program(uint32_t WriteAddr,uint8_t* pBuffer,int16_t NumByteToWrite)
{
...
static rt_size_t rt_qspi_read(rt_device_t dev, rt_off_t sector, void* buffer, rt_size_t count)
{
    ...
    APP_ERROR_CHECK(rt_mutex_take(&flash_lock, RT_WAITING_FOREVER));
    APP_ERROR_CHECK(!count);
   ...
   // for(;count>0;count--)
    ...
    APP_ERROR_CHECK(rt_mutex_release(&flash_lock));
    return cnt;
}
