...
void led_timer_init(void)
{
    ...
    rt_kprintf("LED3 4s后只会执行一次就注销\n");
    rt_kprintf("LED2 会每隔2s循环执行\n");
...
static void led_timer_start(void)
{
    ret_code_t err_code;
    led_timer_init();
    err_code = app_timer_start(led_timer_id1, APP_TIMER_TICKS(4000), NULL);
    APP_ERROR_CHECK(err_code);
    err_code = app_timer_start(led_timer_id2, APP_TIMER_TICKS(2000), NULL);
    APP_ERROR_CHECK(err_code);
...
void led_event_start(void * p_context)
{
   ...
   uint32_t tick_cnt =(uint32_t)rt_tick_get();
   rt_pin_write(ZJ_LED4, pin_toggle); 
   pin_toggle = !pin_toggle;   
   gpio_printf("led_event over %dms\n", TICK_TO_OS_MSEC(ABS(tick_cnt - last_ticks)));
   APP_EVENT_REMOVE_DELAY(led_event_start);
   APP_EVENT_PUSH_DELAY(led_event_start,NULL,500);
   last_ticks = tick_cnt;
}
FINSH_FUNCTION_EXPORT_ALIAS(led_event_start, led_estart, led event start LED4 toggle)
...
FINSH_FUNCTION_EXPORT_ALIAS(led_event_stop, led_estop, led event stop LED4 toggle)
/**
 * @}
 */
